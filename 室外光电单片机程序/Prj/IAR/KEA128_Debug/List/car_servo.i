...
int32 chuizhi_Mid[5] = {0},level_Mid[5] = {0},dip_Left[5] = {0},dip_Right[5] = {0},left_chuizhi[5] = {0},right_chuizhi[5] = {0};  
 ...
 
{
  ...
  SUM_dip = dip_Right[0] + dip_Left[0];   
  square_mid = (0.025) * level_Mid[0] * level_Mid[0]; 
...
{ 
  ...
  servo_error_rate=servo_error[0]-servo_error[2];   
  ...
  servo->kp_1=0.9;
   ...
   servo->pulse_output = servo_error[0] * servo->kp_1 + servo_error_rate * servo->kd;
    ...
    servo->pulse_output += servo->pulse_middle;  
    ...
    if(servo->pulse_output>=servo->pulse_left)
     ...
     {
    ftm_pwm_duty(((FTM_Type *)(0x40039000u)), FTM_CH1, servo->pulse_output);
     
     }  
      else if(servo_state==left_lock)
        ftm_pwm_duty(((FTM_Type *)(0x40039000u)), FTM_CH1, servo->pulse_left);
      else if(servo_state==right_lock)
        ftm_pwm_duty(((FTM_Type *)(0x40039000u)), FTM_CH1, servo->pulse_right);
...
void Straight_deal()
{
  ...
  switch(car_state)
        ...
        if(chuizhi_minus>12 && (level_minus>15 && level_minus<80))    
...
void Cross_deal()
{
 ...
 if((chuizhi_Mid[0]>=90&&chuizhi_Mid[0]<=180) && cha_lr>10)   
...
{ 
 ...
 static int right_turn=0,left_turn=0;
  ...
  if(SUM_LRL>SUM_MAX)
   ...
   if(SUM_MAX>= 420 && chuizhi_Mid[0]<150)  
  {
  ...
  if(shibie_ring)
  {
   ...
   if(right_chuizhi[0]>125 && chuizhi_Mid[0]<=30&&left_chuizhi[0]>65)     
  ...
  if(oneflag)                    
  {
   ...
   if(right_curve==1 && dip_Right[0]>220)         
      {
        Rcurve++;
        servo_error[0]=75;            
        motor.set_speed=300;
...
void lose_line()
{
  ...
  {  
    ftm_pwm_duty(((FTM_Type *)(0x40038000u)),FTM_CH1,9100);  
