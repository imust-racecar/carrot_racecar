...
uint32 sum=0;  
uint16 i=0,j=0;
uint32 senser[(6)][20]={0};
  for(i=0;i<20;i++)      
  {
  senser[0][i]=adc_once((ADC0_SE0),ADC_8bit);
  
  senser[1][i]=adc_once((ADC0_SE1), ADC_8bit); 
  senser[2][i]=adc_once((ADC0_SE2), ADC_8bit);
  senser[3][i]=adc_once((ADC0_SE4 ),ADC_8bit);
  senser[4][i]=adc_once((ADC0_SE8),ADC_8bit);
  senser[5][i]=adc_once((ADC0_SE9),ADC_8bit);
  }

  for(i=0;i<6;i++)             
  {
       bubble((uint32*)&senser[i],20);    
 ...
 Senser_ADC->ad_act_val[i][0] = sum / 18;    
...
void time_flag_5ms(ADC_info  *Senser_ADC,Servo_pid_info *servo)
{
  ...
  for(uint8 i=0;i<(6);i++)
  {
      Senser_ADC->ad_avr_val[i] = Senser_ADC->ad_act_val[i][0] * 0.5                              + Senser_ADC->ad_act_val[i][1] * 0.2                              + Senser_ADC->ad_act_val[i][2] * 0.1                              + Senser_ADC->ad_act_val[i][3] * 0.1                              + Senser_ADC->ad_act_val[i][4] * 0.1 ;
   
  }

 
  
  Servo_identify(Senser_ADC); 
